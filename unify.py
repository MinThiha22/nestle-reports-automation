...
def get_notification_items(page):
  notifications = []
  target_files = ["Flat File - CD", "Flat File - NWNI", "Flat File - NWNI", "Flat File - PSNI", "Flat File - FSSI", "Flat File - Petrol CDNISI", "Flat File - Petrol CDNISI", "Flat File - TSM NI/SI", "Flat File - Chemist Warehouse"]
  # Open notifications list
  locate_and_action(page, 'a.fa-bell', description='Noti Bell Icon')
  locate_and_action(page, 'a.fa-expand span', has_text='View All', description='View All')

  try:
    # Wait for notifications to appear
    page.wait_for_selector('div.cdk-virtual-scroll-content-wrapper div.row', timeout=5000)
    
    # Get all notifications
    notification_elements = page.locator('div.cdk-virtual-scroll-content-wrapper div.row').all()
    filtered_notifications = []
    
    for element in notification_elements:
      try:
        title_span = element.locator('div.title span.ellipsis')
        file_name = title_span.get_attribute('title') or title_span.inner_text()

        if file_name.strip() in target_files:
          time_div = element.locator('div.col-2').nth(0)
          time_str = time_div.inner_text()
  ...
  for file in os.listdir(DOWNLOADS_PATH):
    ...
    if os.path.isfile(file_path) and uuid_pattern.match(os.path.splitext(file)[0]):
      ...
      if file_modified_time == today:
        try:
          os.remove(file_path)
          print(f"🗑️ Deleted UUID file: {file}")
        except Exception as e:
